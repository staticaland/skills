...
Verify any setting against the installed mise before writing it: `mise settings --all | grep <name>`, then `mise settings get <name>`. Full list: <https://mise.jdx.dev/configuration/settings.html>
...
Prefer exact versions with `lockfile = true`, and write them with `mise use --pin`. `latest` and a bare prefix re-resolve on a new machine, which is the drift the lockfile exists to prevent.
