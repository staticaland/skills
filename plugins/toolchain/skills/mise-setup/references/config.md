...
| `locked = true`                                                | `mise install` fails when the lockfile has no pre-resolved URL for the platform, so no install calls GitHub or the aqua registry. Invocation-wide, includes global and system tools. Prefer `[tool_config] locked = true` in a project |
| `[tool_config] locked = true`                                  | Enforce the lockfile for the tools declared in this config root only. Its own table, not `[settings]`. mise 2026.8.6 or newer                                                                                                          |
| `locked_scopes = ["project"]`                                  | Global-only. Limits `--locked` and `MISE_LOCKED` to the listed scopes: `project`, `global`, `system`. mise 2026.9.1 or newer. Suggest to a user whose global tools trip `--locked`                                                     |
