...
Native cooldown since **4.0.13**. The value is always a non-negative integer
number of days - a string, float, negative number, or array is rejected.
...
Override hierarchy: command-line flag > configuration setting > per-source
`Gemfile` declaration.

Bypass for a whole run with `bundle install --cooldown 0`. There is no per-gem
exemption, but a per-source override exempts everything from one registry:
